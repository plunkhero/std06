...
# 1. สร้างพจนานุกรมคำพ้องความหมาย Data Augmentation
...
def augment_legal_text(text):
    ...
    for i , word in enumerate(words):
        if word in LEGAL_SYNONYMS:
            new_word[i] = random.choice(LEGAL_SYNONYMS[word])
...
def balance_legal_data(X,y):
    counts = Counter(y)
    print(f"Original distrubutetion : {counts}")
    # คลาสน้อยสุดมีกี่ตัว
    min_sample = min(counts.values())
    if min_sample > 1 :
        sampler = SMOTE(k_neighbors=min(5, min_sample-1),random_state=6)
    else:
        sampler = RandomOverSampler(random_state=6)
    
    X_res , y_res = sampler.fit_resample(X,y)
    print(f"Balanced distrubutetion: {Counter(y_res)}")
    return X_res, y_res

# จำลองข้อมูล Imbabance (class)
...
model = LegalBiLSTM()
sample_input = torch.randn(1,5,16) #1 doc : 5 token : 16 dimention
output = model(sample_input)
print(f"--BiLSTM Output---")
print(f"Logistics: {output.detach().numpy()}") 